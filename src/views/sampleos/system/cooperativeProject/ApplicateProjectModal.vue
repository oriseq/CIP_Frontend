...
<script lang="ts" setup>
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    updateSchema([
      {
        field: 'applyProject',
        ifShow: data?.moadlType == MoadlType.APPLY_PROJECT,
      },
    ]);
    // console.log('useModalInner:data:', data);

    /** 申请项目 */
    if (moadlType.value == MoadlType.APPLY_PROJECT) {
      // 1. 项目树获取
      const projectTree = await getProjectTreeApi(null);
      // console.log('projectTree-show', projectTree);
      // treeData = data;
      treeData.splice(0, treeData.length, ...projectTree);
      // 2. 获取当前表格的项目，用于判断申请项目是否为新项目
      let projects = data.projects;
      // console.log('projects: ', projects);
      // 3. 勾选的项目信息
      const selectedIds = projects.map((item) => item.originProjectId);
      ...
      projects.forEach((element) => {
        ...
        const foundObject = findObjectById(treeData, element.originProjectId);
        // console.log('foundObject: ', foundObject); // eg. {id: 188, name: 'NK细胞检测（CD3/CD16+56）', status: 2, consumableType: '紫头管'}
        if (foundObject) {
          // foundObject.status = element.projectStatus; // 没有projectStatus属性
          // 加上不变
  ...
  async function handleSubmit() {
    try {
      ...
      if (moadlType.value == MoadlType.APPLY_PROJECT) {
        ...
        const formData = { projectIds: selectProjectIdsList };
        // console.log('formData:', formData);

        // 发请求
        const data = await applyProjectApi(formData);
