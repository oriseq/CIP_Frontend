      ...
      <template v-for="item in settingList" :key="item.key">
        <TabPane :tab="item.name">
          ...
          <ScrollContainer style="width: 100%; max-width: 100%">
            <component :is="tabs[item.component]" :dataArray="allData" />
          </ScrollContainer>
        </TabPane>
      </template>
    </Tabs>
...
<script lang="ts" setup>
  ...
  import { settingList, getInfoApi } from './data';
  ...
  onMounted(() => {
    getInfoApi(null).then((res) => {
      console.log('🚀 ~ getInfoApi: res:', res);
      allData.value = res.result;
    });
  });

  const prefixCls = 'background-setting';
</script>
<style lang="less">
  // 定义左侧宽度
  @tab-bar-width: 150px;
