        ...
        <List.Item.Meta>
          <template #title>
            <div class="title">
              ...
              <div class="extra" v-if="item.type == MessageType.NOTIFICATION">
                <a-button size="small" @click="handleTitleClick(item)">查收</a-button>
              </div>
              <div style="width: 80%">
...
<script lang="ts" setup>
  import { computed, PropType, ref, watch, unref, onMounted } from 'vue';
  ...
  const emit = defineEmits(['update:currentPage', 'update:getData']);
  ...
  const getData = computed(() => {
    ...
    const pageContent = list.slice(size * (unref(current) - 1), size * unref(current));
