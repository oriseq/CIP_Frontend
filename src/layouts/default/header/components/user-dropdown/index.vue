  ...
  <Dropdown placement="bottomLeft" :overlayClassName="`${prefixCls}-dropdown-overlay`">
    <span :class="[prefixCls, `${prefixCls}--${theme}`]" class="flex">
      ...
      <span :class="`${prefixCls}__info hidden md:block`">
        <span :class="`${prefixCls}__name`" class="truncate">
          <!-- {{ getUserInfo.realName }} // realName并未取到值，参考loginForm.vue，改为username -->
    ...
    <template #overlay>
      <Menu @click="handleMenuClick">
        ...
        <MenuItem
          key="setting"
          :text="t('layout.header.Personsetting')"
          icon="ant-design:setting-outlined"
        />
        <MenuItem
          key="backgroundSetting"
          :text="t('layout.header.BackGroundSetting')"
...
<script lang="ts" setup>
  ...
  type MenuEvent = 'logout' | 'doc' | 'lock' | 'api' | 'setting' | 'backgroundSetting';
  ...
  // 前往后台管理面板
  function goBackgroundSetting() {
    go('/backgroundSetting');
  }
</script>
<style lang="less">
